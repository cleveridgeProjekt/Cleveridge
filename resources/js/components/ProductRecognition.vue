<template>
    <div>
        <PageHeader :title="$t('product_recognition.title')" icon="fal fa-barcode">
            {{ $t('product_recognition.intro') }}
        </PageHeader>

        <div class="card toolbar">
            <div class="status-area">
                <div v-if="waitingForPi" class="status-badge blink">
                    <i class="fas fa-circle-notch fa-spin"></i> {{ $t('product_recognition.status.waiting') }}
                </div>
                <div v-else class="status-badge ready">
                    <i class="fas fa-check-circle"></i> {{ $t('product_recognition.status.ready') }}
                </div>
            </div>

            <div class="actions">
                <button class="btn" @click="triggerPiCamera" :disabled="waitingForPi">
                    <i class="fas fa-camera"></i> {{ $t('product_recognition.buttons.trigger_photo') }}
                </button>
            </div>
        </div>
        ...
        <div class="content-grid">
            <div class="photo-card card">
                <h3>📸 {{ $t('product_recognition.sections.last_photo') }}</h3>
                <div class="image-wrap">
                    <img v-if="latestImage" :src="latestImage" alt="Raspberry Pi Capture" class="pi-photo"/>
                    <div v-else class="placeholder">
                        <i class="fas fa-image"></i>
                        <p>{{ $t('product_recognition.placeholder.no_photo') }}</p>
                    </div>
                </div>
            </div>

            <div class="results-card card">
                <h3>🛒 {{ $t('product_recognition.sections.detected_products') }}</h3>
                ...
                <div v-if="detectedProducts.length > 0" class="product-list">
                    <div v-for="(item, index) in detectedProducts" :key="index" class="product-row">
                        ...
                        <button class="btn small ghost" @click="openAddModal(item)">
                            <i class="fas fa-plus"></i> {{ $t('product_recognition.buttons.add') }}
                        </button>
                    </div>
                </div>

                <div v-else class="empty-state">
                    <p>{{ $t('product_recognition.empty.none') }}</p>
                </div>
            </div>
        </div>
        ...
        <div v-if="modal.open" class="overlay" @click.self="closeModal">
            <div class="modal">
                <div class="modal-head">
                    <h3>{{ $t('product_recognition.modal.title') }}</h3>
                    <button class="icon-btn" @click="closeModal"><i class="fas fa-times"></i></button>
                </div>

                <div class="modal-grid">
                    <div class="col">
                        <label class="lbl">{{ $t('product_recognition.modal.detected_label') }}</label>
                        <div class="chip">{{ modal.label }}</div>

                        <label class="lbl">{{ $t('product_recognition.modal.mapped_product') }}</label>
                        <div v-if="modal.product">
                            ...
                            <button class="btn ghost small" @click="modal.product=null">{{ $t('product_recognition.modal.relink') }}</button>
                        </div>
                        <div v-else class="relink">
                            <input class="ui-input" v-model="modal.search" :placeholder="$t('product_recognition.modal.search_placeholder')"/>
                    ...
                    <div class="col">
                        <label class="lbl">{{ $t('product_recognition.modal.target_fridge') }}</label>
                        <select class="ui-select" v-model="selectedFridgeId">
                            <option v-for="f in fridges" :key="f.id" :value="f.id">{{ f.name || $t('fridge.list.unnamed') }}</option>
                        </select>

                        <div class="split">
                            <div>
                                <label class="lbl">{{ $t('product_recognition.modal.qty') }}</label>
                                <input class="ui-input" type="number" min="1" v-model.number="modal.qty"/>
                            </div>
                            <div>
                                <label class="lbl">{{ $t('product_recognition.modal.expiry') }}</label>
                        ...
                        <div class="actions">
                            <button class="btn" :disabled="!modal.product || !selectedFridgeId || busyAdd" @click="addToFridge()">
                                <i class="fas fa-plus"></i> {{ $t('product_recognition.modal.add') }}
                            </button>
                            <button class="btn ghost" @click="closeModal">{{ $t('product_recognition.modal.cancel') }}</button>
                        </div>
                    </div>
                </div>
            </div>
